{
  description = "KOReader macOS mirror — pure nix packages from GitHub releases";
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      mkPackages = pkgs: {
        koreader = pkgs.callPackage
          (mkKoreader {
            pname = "koreader";
            inherit (sources.koreader) version arm64 x86_64;
          })
          { };
        koreader-nightly = pkgs.callPackage
          (mkKoreader {
    ...
    in
    {
      ...
      packages = {
        aarch64-darwin = mkPackages (import nixpkgs { system = "aarch64-darwin"; })
          // { default = self.packages.aarch64-darwin.koreader; };
        x86_64-darwin = mkPackages (import nixpkgs { system = "x86_64-darwin"; })
